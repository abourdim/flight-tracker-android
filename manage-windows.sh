#!/usr/bin/env bash
# ============================================================================
#  Flight Tracker Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-10-flight-tracker}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.flighttracker}"
AVD_NAME="${AVD_NAME:-flight_tracker_test}"
...
cmd_test() {
  ...
  local out="/tmp/flight-tracker-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Flight Tracker Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
